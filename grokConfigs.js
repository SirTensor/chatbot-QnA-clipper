// --- START OF FILE grokConfigs.js ---

/**
 * Configuration for extracting Q&A data from Grok (grok.com)
 * Version: 6 (Handle nested lists correctly)
 */
(function() {
    // Initialization check
    if (window.grokConfig && window.grokConfig.version >= 6) { // Updated version check
     ...
     * **Updated in v6:** Detects and formats nested lists and code blocks directly.
     * Handles standard inline markdown elements (strong, em, a, br) and Grok's inline code spans.
    ...
    function processNode(node) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          if (node.matches(selectors.assistantCodeBlockOuterContainer)) {
              console.log("  [processNode v6] Handling nested Code Block (div.not-prose)");
          ...
          if (node.matches(selectors.assistantImageGrid)) {
              console.log("  [processNode v6] Handling nested Image Grid");
          ...
          // *** ADDED in v6: Handle nested lists ***
          if (tagName === 'ul' || tagName === 'ol') {
              console.log("  [processNode v6] Handling nested List <" + tagName + ">");
          ...
          // Note: 'ul'/'ol' check is now above this, so shouldSkipElement doesn't need to check for them.
    ...
    const grokConfig = {
      platformName: 'Grok',
      version: 6, // Updated config version
      selectors: {
        ...
        assistantRelevantBlocks: ':scope > :is(p, h1, h2, h3, h4, h5, h6, ol, ul, div.not-prose, div.grid)',
        listItem: 'li',
      ...
      /**
       * Extracts structured content items (text, code, images, lists) from an assistant's message bubble.
       * Iterates through relevant block-level elements within the bubble and processes them accordingly.
       * **Updated in v6:** Uses processNode which now handles nested blocks (lists, code, images) within list items etc.
      ...
      extractAssistantContent: (turnElement) => {
        ...
        console.log(`[Grok Extractor v6] Processing assistant message bubble.`);
        ...
        relevantBlocks.forEach((block, index) => {
            ...
            console.log(`[Grok Extractor v6] Processing Block #${index}: <${tagNameLower}>`);
            ...
            // --- Fallback / Unhandled ---
                ...
                console.warn(`[Grok Extractor v6]   -> Skipping unhandled direct block type <${tagNameLower}>`, block);
                processedElements.add(block);
            }
        }); // End forEach loop over relevantBlocks
  
        console.log("[Grok Extractor v6] Final assistant contentItems:", JSON.stringify(contentItems, null, 2));
